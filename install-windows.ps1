...
if ($distros -match "Ubuntu") {
    ...
    if ([string]::IsNullOrEmpty($cont) -or $cont -eq "i") {
        ...
        wsl -d Ubuntu -- bash -c "curl -fsSL https://raw.githubusercontent.com/MRXH4X/wraith/main/install-linux.sh -o /tmp/wraith-install.sh && bash /tmp/wraith-install.sh"
    ...
    Write-Host "    curl -fsSL https://raw.githubusercontent.com/MRXH4X/wraith/main/install-linux.sh -o install.sh && bash install.sh" -ForegroundColor Cyan
...
if [ ! -d "\$INSTALL_DIR" ]; then
    git clone --branch main https://github.com/MRXH4X/wraith.git "\$INSTALL_DIR"
    echo '  ✓ Repó klónozva'
else
