...
window._i18n.en = {
  ...
  'agents.wraith_process_tip':    'Running: the main assistant always runs in the --channels session. This card always shows Running -- no per-agent tmux check.',
  'agents.wraith_channel_tip':    'Online: the main assistant channel is managed by the --channels session, so it is always online (no separate token check).',
  ...
  'agents.toast.wraith_restarted': '{bot} channels restarted',
  ...
  'onboarding.identity.saved_restart_failed': 'Name saved, but restarting the agent failed. Restart it manually (Linux: systemctl --user restart wraith-channels), then come back here.',
  'onboarding.identity.saved_restart_needed': 'Name saved. The running agent picks up its new name on its next restart.',
  'onboarding.title':            'Set up Wraith',
  ...
  'onboarding.step1.saved_restart_failed': 'Token saved, but restarting the agent failed. Restart it manually (Linux: systemctl --user restart wraith-channels), then come back here.',
  ...
  'auth.devices.revoke_ssh_warning': 'Device key revoked, but the SSH entry could not be removed -- the device can still open the tunnel. Delete the matching wraith-remote line from ~/.ssh/authorized_keys manually (details: docs/dashboard-auth-recovery.md).',

  'auth.bridge.title':             'Bridge pairing',
  'auth.bridge.desc':              'Paste the key line shown by the Bridge app (ssh-ed25519 ... wraith-remote:...), name the device, then copy the returned bundle back into the Bridge. The device gets its own, individually revocable key.',
