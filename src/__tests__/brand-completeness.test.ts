...
// Stock defaults (BRAND_NAME == BOT_NAME == 'Wraith', slug 'wraith') and a
...
const DEFAULT_TOKENS = { brand: 'Wraith', bot: 'Wraith', agentId: 'wraith' }
...
describe('buildManifest brands the PWA manifest, default unchanged', () => {
  ...
  it('keeps the stock name/short_name with the default brand', () => {
    const m = JSON.parse(buildManifest(raw, 'Wraith'))
    expect(m.name).toBe('Wraith Dashboard')
    expect(m.short_name).toBe('Wraith')
  })
  ...
  it('serves byte-for-byte the shipped file when brand is the default', () => {
    ...
    expect(buildManifest(raw, 'Wraith')).toBe(raw)
  })
...
describe('buildGateMsg brands the email-gate deny message', () => {
  ...
  it('substitutes a custom brand + owner and drops the stock names', () => {
    ...
    expect(msg).not.toContain('Wraith')
...
describe('i18n brand tokens keep a stock install byte-identical (en)', () => {
  it('renders the original English strings with default tokens', async () => {
    ...
    expect(substitute(en['updates.brand_subtitle'], DEFAULT_TOKENS)).toBe('Wraith version check')
    expect(substitute(en['agents.wraith_boss'], DEFAULT_TOKENS)).toBe('Wraith Boss')
    expect(substitute(en['agents.toast.wraith_restarted'], DEFAULT_TOKENS)).toBe('Wraith channels restarted')
    expect(substitute(en['agents.confirm.hard_restart'], DEFAULT_TOKENS)).toBe(
      'Hard restart the wraith-channels session. The ongoing Wraith conversation will be lost (memory is preserved). Continue?',
    )
    expect(substitute(en['settings.desc.DASHBOARD_PUBLIC_URL'], DEFAULT_TOKENS)).toContain('https://wraith.example.com')
    expect(substitute(en['connectors.builtin.computer_use_html'], DEFAULT_TOKENS)).toContain('not by Wraith.')
    expect(substitute(en['connectors.builtin.chrome_html'], DEFAULT_TOKENS)).toContain('Wraith sub-agent launches')
  })

  it('carries the brand/agent names with non-default tokens and drops Wraith', async () => {
    const en = await loadLang('en')
    expect(substitute(en['agents.wraith_boss'], BRANDED_TOKENS)).toBe('Nova Boss')
    ...
    expect(restart).not.toMatch(/wraith/i)
    expect(substitute(en['connectors.builtin.computer_use_html'], BRANDED_TOKENS)).not.toMatch(/wraith/i)
...
describe('i18n brand tokens keep a stock install byte-identical (hu)', () => {
  it('renders the original Hungarian strings with default tokens', async () => {
    ...
    expect(substitute(hu['updates.brand_subtitle'], DEFAULT_TOKENS)).toBe('Wraith verzió ellenőrzés')
    expect(substitute(hu['agents.wraith_boss'], DEFAULT_TOKENS)).toBe('Wraith Főnök')
    expect(substitute(hu['agents.toast.wraith_restarted'], DEFAULT_TOKENS)).toBe('Wraith channels újraindítva')
    expect(substitute(hu['agents.confirm.hard_restart'], DEFAULT_TOKENS)).toBe(
      'Hard restart a wraith-channels session-ön. A folyamatban lévő Wraith beszélgetés elveszik (memória megmarad). Folytatod?',
    )
  })

  it('carries the brand/agent names with non-default tokens and drops Wraith', async () => {
    const hu = await loadLang('hu')
    expect(substitute(hu['agents.wraith_boss'], BRANDED_TOKENS)).toBe('Nova Főnök')
    expect(substitute(hu['updates.brand_subtitle'], BRANDED_TOKENS)).toBe('Acme verzió ellenőrzés')
    expect(substitute(hu['agents.confirm.hard_restart'], BRANDED_TOKENS)).not.toMatch(/wraith/i)
  })
})
