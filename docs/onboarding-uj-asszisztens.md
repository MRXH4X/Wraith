# Új asszisztens onboarding (Wraith flotta)

Ez a leírás végigvezet egy új kolléga-asszisztens beüzemelésén: saját Telegram bot
és saját Google (Gmail/Drive/Naptár) hozzáférés. Példa: "Dia Wraithja".
...
- **A személyes Google a kolléga saját asszisztensébe kerül**, nem a Főnökbe
  (iS Wraith Főnök / wraith-is). A Főnök a flottát kezeli, nem olvassa senki
  postáját.
...
1. **Bot létrehozása (Telegramban, @BotFather):**
   ...
   - Név: pl. `Dia Wraithja`
   - Username: pl. `dia_wraith_bot` (egyedinek kell lennie, `_bot`-ra végződik)
...
2. **Asszisztens létrehozása a dashboardon** (https://wraith.isolutions.hu):
...
4. **A kolléga hozzáférése (párosítás):**
   - A kolléga megnyitja a botot (`t.me/dia_wraith_bot`), és ír neki egy üzenetet.
...
A kolléga bekötése:
1. A Főnök (iS Wraith Főnök) előkészíti az asszisztens Google-konfigját és
...
| Google konfig + lezárás a szerveren | iS Wraith Főnök |
...
*Készítette: iS Wraith Főnök. A flotta a wraith.isolutions.hu címen érhető el.*
